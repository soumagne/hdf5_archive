...
//            HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL, "not index access parms");
    ...
    /* Apply query */
    if (NULL == (ret_grp = H5Q_apply(&loc, query, result, vcpl_id)))
        HGOTO_ERROR(H5E_QUERY, H5E_CANTCOMPARE, FAIL, "unable to apply query");
 ...
 */
H5G_t *
H5Q_apply(const H5G_loc_t *loc, const H5Q_t *query, unsigned *result,
    hid_t H5_ATTR_UNUSED vcpl_id)
{
    ...
    if (FAIL == H5O_visit(loc, ".", H5_INDEX_NAME, H5_ITER_NATIVE, H5Q__apply_iterate,
...
//            HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL, "not index access parms");
    ...
    /* Apply query */
    if (NULL == (ret_grp = H5Q_apply_multi(loc_count, locs, query, result, vcpl_id)))
        HGOTO_ERROR(H5E_QUERY, H5E_CANTCOMPARE, FAIL, "unable to apply query");
 ...
 */
H5G_t *
H5Q_apply_multi(size_t loc_count, const H5G_loc_t *locs, const H5Q_t *query,
    unsigned *result, hid_t H5_ATTR_UNUSED vcpl_id)
{
    ...
    for (i = 0; i < loc_count; i++) {
        ...
        if (FAIL == H5O_visit(&locs[i], ".", H5_INDEX_NAME, H5_ITER_NATIVE, H5Q__apply_iterate,
                &args, H5P_LINK_ACCESS_DEFAULT, H5AC_ind_dxpl_id))
            HGOTO_ERROR(H5E_SYM, H5E_BADITER, NULL, "object visitation failed");
