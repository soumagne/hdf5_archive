...
//    }
//
//    if(H5R_encode(H5R_REGION, NULL, &ref->buf_size, opened_obj, space_id, NULL) < 0)
//        HGOTO_ERROR(H5E_REFERENCE, H5E_CANTENCODE, FAIL, "Can't determine buffer size to encode reference");
//
//    if(NULL == (ref->buf = HDmalloc(ref->buf_size)))
//        HGOTO_ERROR(H5E_SYM, H5E_NOSPACE, FAIL, "can't allocate reference buffer");
//
//    if(H5R_encode(H5R_REGION, ref->buf, &ref->buf_size, opened_obj, space_id, NULL) < 0)
//        HGOTO_ERROR(H5E_REFERENCE, H5E_CANTENCODE, FAIL, "Can't determine buffer size to encode reference");
//
 ...
 */
hid_t
H5Rget_object_ff(hid_t loc_id, hid_t H5_ATTR_UNUSED oapl_id, href_t _ref,
    hid_t rcxt_id, hid_t estack_id)
{
    ...
    if(ref == NULL)
        HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL, "invalid reference pointer");
    if(ref->ref.serial.buf == NULL || ref->ref.serial.buf_size == 0)
    ...
    if(ref->ref_type == H5R_OBJECT || ref->ref_type == H5R_REGION ||
       ref->ref_type == H5R_ATTR) {
        size_t token_size;
...
//    if(ref->buf == NULL || ref->buf_size == 0)
//        HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL, "invalid reference");
//    if(ref->ref_type != H5R_EXT_OBJECT && ref->ref_type != H5R_EXT_REGION &&
//       ref->ref_type != H5R_EXT_ATTR)
//        HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL, "invalid reference type");
//
//    p = (uint8_t *)ref->buf;
...
//    HDassert(ref_type == H5R_OBJECT || ref_type == H5R_REGION ||
//             ref_type == H5R_ATTR);
//
...
//
//    HDcompile_assert(sizeof(size_t) <= sizeof(uint64_t));
//    HDassert(ref_type == H5R_EXT_OBJECT || ref_type == H5R_EXT_REGION ||
//             ref_type == H5R_EXT_ATTR);
//    HDassert(filename);
//    HDassert(pathname);
//
